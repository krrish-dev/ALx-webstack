...
export default function Chat() {
  ...
  const { user, logout, updateUser, loading } = useContext(AuthContext); // Add loading from AuthContext
  ...
  console.log('Received online users:', users); // Log the data received
  setOnlineUsers(users); // Update the onlineUsers list with the latest data
});
      ...
      try {
        ...
        if (updatedUser) {
          ...
          if (file) {
            ...
            const profilePictureResponse = await uploadProfilePicture(user._id, formData);
            ...
            // If no file is uploaded, update the user's profile data in the context
            const updatedUserData = { ...user, username, email, bio };
            updateUser(updatedUserData);
  ...
  const handleUserClick = (user) => {
    ...
    setShowUserModal(true); // Show the user modal
  };
    ...
    {onlineUsers && onlineUsers.length > 0 ? (
      onlineUsers.map((user) => (
        <li
          ...
          onClick={() => handleUserClick(user)} // Open the modal when clicked
        >
          <img
